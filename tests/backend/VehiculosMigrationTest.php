<?php
/**
 * tests/backend/VehiculosMigrationTest.php
 *
 * Tests de migración para vehiculos-migrated.php
...
class VehiculosMigrationTest extends TestCase
{
    ...
    {
        $this->apiPath = __DIR__ . '/../../api/vehiculos-migrated.php';
    ...
    {
        $this->assertFileExists($this->apiPath, 'vehiculos-migrated.php debe existir');
    }
    ...
    /**
     * Test 7: Verificar manejo de archivos/documentos
     */
    public function testHandlesDocuments(): void
    {
        $content = file_get_contents($this->apiPath);
        ...
        $this->assertStringContainsString(
            'permiso',
            $content,
            'Debe manejar permisos'
        );

        $this->assertStringContainsString(
            'certificado',
            $content,
            'Debe manejar certificados'
        );
    }
    ...
    public function testTableExists(): void
    {
        try {
            ...
            $result = $conn->query("SELECT 1 FROM vehiculos LIMIT 1");
            $this->assertNotFalse($result, 'Tabla vehiculos debe existir');
